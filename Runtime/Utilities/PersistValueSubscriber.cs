...
namespace Framework.Runtime.Utilities
{
  public class PersistValueSubscriber<T> : ValueSubscriber<T> where T : IEquatable<T>
  {
    ...
    public PersistValueSubscriber(Lifetime lifetime, Persist persistence, T defaultValue) : this(lifetime, persistence, defaultValue, new Converter<T>(a => a, a => a)) { }

    public PersistValueSubscriber(Lifetime lifetime, Persist persistence, T defaultValue, IConverter converter) : base(lifetime, defaultValue)
    {
    ...
    public class Converter<TPersist> : IConverter
    {
      ...
      {
        return _deserializer((TPersist)value);
      }
